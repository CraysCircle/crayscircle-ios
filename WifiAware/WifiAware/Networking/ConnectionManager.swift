...
typealias WiFiAwareConnectionState = (WiFiAwareConnection, NWConnection.State)
...
actor ConnectionManager: Sendable {
    private var connections: Set<WiFiAwareConnection> = []
    private var connectionsInfo: [WiFiAwareConnection.ID: ConnectionInfo] = [:]
...
// MARK: - Monitor
    ...
    func monitor() async throws {
        for connection in connections.filter({ $0.state == .ready }) {
            if let wifiAwarePath = try await connection.currentPath?.wifiAware {
                let connectedDevice = wifiAwarePath.endpoint.device
                let performanceReport = wifiAwarePath.performance
...
// MARK: - Teardown
    ...
    func invalidate(_ id: WiFiAwareConnection.ID) {
